...
A canonical pair-Planner PLAN declares a cross-root design edge by carrying all seven fields `DESIGN_SOURCE_REPO`, `DESIGN_SOURCE_COMMIT`, `DESIGN_SOURCE_ROOT`, `DESIGN_SOURCE_PATH`, `DESIGN_OWNER`, `DESIGN_DOC_ID`, and `DESIGN_SHA256`; occurrence of any source or owner field engages the gate, and the declaration is all-or-none with a non-empty `DESIGN_LOCK_ID` and `DESIGN_RECORD_KIND: design-doc`.
...
Path-form `PLAN_LOCK_ID` gains the symmetric declaration `PLAN_SOURCE_REPO` plus `PLAN_SOURCE_COMMIT`; occurrence of either engages the same conflict-first repository, commit, literal-path, blob, and optional annotation-digest checks with no local-path fallback.
