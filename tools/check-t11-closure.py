...
TRAILING_REVIEW_STAMP_RE = re.compile(r"-(\d{8}-\d{6})$")
...
def review_values(linter, path: Path, text: str, review_parent: str) -> tuple[dict[str, str] | None, int | None]:
    values: dict[str, str] = {}
    for key in (
        "DISPATCH_ID",
        "PHASE",
        ...
        value, error = one_distinct_value(linter, text, "review relay", key)
    ...
    for key, want in (
        ...
        error = require_value("review relay", key, values[key], want)
        if error is not None:
            return None, error
    if not exact_lint_ok(linter, path):
        return None, refuse("review relay fails exact-file lint")
    return values, None


def currency_candidates(linter, cycle_dir: Path, review_parent: str) -> list[tuple[Path, str]]:
    ...
    for path in sorted(cycle_dir.rglob("*.md")):
        ...
        text = linter.read(path)
        if (
            "PLAN-REVIEW" in occurrences(linter, text, "PHASE")
            and review_parent in occurrences(linter, text, "PARENT_DISPATCH_ID")
        ):
            candidates.append((path, text))
    return candidates


def currency_stamp(linter, path: Path) -> str | None:
    status, _stamp, raw, _is_utc = linter.filename_timestamp(path.name)
    trailing = TRAILING_REVIEW_STAMP_RE.search(path.stem)
    if status != "ok" or trailing is None or raw != trailing.group(1):
        return None
    return raw
...
def run_gate(
    ...
    *,
    ...
    try:
        ...
        named_review_values, error = review_values(
            linter, review_path, review_text, review_parent
        )
        if error is not None:
            return error
        if named_review_values is None:
            return refuse("review relay authentication did not produce values")
        error = require_value(
            "review relay", "DISPATCH_ID", named_review_values["DISPATCH_ID"], review_id
        )
        if error is not None:
            return error

        if parent_values["PLAN_LOCK_ID"] != named_review_values["PLAN_LOCK_ID"]:
            return refuse("parent PLAN_LOCK_ID does not match the approving review's PLAN_LOCK_ID")

        authenticated_reviews: list[tuple[Path, dict[str, str], str]] = []
        for candidate_path, candidate_text in currency_candidates(
            linter, closing.parent, review_parent
        ):
            candidate_values, error = review_values(
                linter, candidate_path, candidate_text, review_parent
            )
            if error is not None:
                return error
            if candidate_values is None:
                return refuse("review relay authentication did not produce values")
            stamp = currency_stamp(linter, candidate_path)
            if stamp is None:
                return refuse(
                    f"review candidate {candidate_path.name!r} carries no parseable stamp; "
                    "currency cannot be ordered"
                )
            authenticated_reviews.append((candidate_path, candidate_values, stamp))

        latest_stamp = max(stamp for _path, _values, stamp in authenticated_reviews)
        latest_reviews = [
            (path, values)
            for path, values, stamp in authenticated_reviews
            if stamp == latest_stamp
        ]
        if len(latest_reviews) != 1:
            return refuse(
                f"{len(latest_reviews)} authenticated approving reviews share the latest stamp; "
                "required a unique latest"
            )
        _latest_path, latest_values = latest_reviews[0]
        if latest_values["PLAN_REVIEW_VERDICT"] != "approve":
            return refuse(
                "latest review on the lineage is "
                f"{latest_values['PLAN_REVIEW_VERDICT']!r}; closure requires 'approve'"
            )

        lock_match = LOCK_RE.fullmatch(latest_values["PLAN_LOCK_ID"])
        ...
        if sha256(resolved_plan) != declared_plan_sha:
            return refuse("latest approving review lock does not match plan bytes on disk")
...
def candidate_discovery_selftest(root: Path, payload_name: str) -> int:
    """Ensure conflicting first values cannot hide a review-lineage candidate."""
    with tempfile.TemporaryDirectory(prefix="t11candidate.") as tmp:
        copied = Path(tmp) / "G1-valid"
        shutil.copytree(root / "G1-valid", copied)
        shutil.copy2(
            root / payload_name,
            copied / "PLAN-REVIEW-implementer-20260805-000001.md",
        )
        ...
        expected = (
            root / f"{Path(payload_name).stem}.expected-reason.txt"
        ).read_text(encoding="utf-8")
        if status != 0 and stdout.getvalue() == "" and stderr.getvalue() == expected:
            return 0
        print(
            f"selftest: {Path(payload_name).stem}: "
            f"status={status} stdout={stdout.getvalue()!r} stderr={stderr.getvalue()!r}",
            file=__import__("sys").stderr,
        )
        return 1


def selftest(root: Path, only: str | None = None, expect_pass: bool = False) -> int:
    fixtures = [
        root / "G1-valid",
        root / "G2-duplicate-identical-verdict",
        root / "G3-superseding-approve",
    ...
    if only is not None:
        ...
        candidate_discovery_selftest(root, payload_name) != 0
        for payload_name in (
            "currency-candidate-first-value-conflict.md",
            "currency-candidate-first-value-lineage-conflict.md",
        )
    ):
        return 1
    for fixture in fixtures:
        with tempfile.TemporaryDirectory(prefix="t11closure.") as tmp:
            ...
            if fixture.name.startswith("R26-"):
                ...
                for name in (
                    "parent.md",
                    "PLAN-REVIEW-implementer-20260805-000000.md",
                ):
                    path = copied / name
                    path.write_text(
            ...
            if only is not None and expect_pass:
            ...
            if fixture.name in {
                "G1-valid",
                "G2-duplicate-identical-verdict",
                "G3-superseding-approve",
            }:
                good = (
                    status == 0
...
def main() -> int:
    ...
    if args.selftest:
        return selftest(args.selftest, args.only, args.expect_pass)
    if not all(
        (
            args.closure,
            args.closing,
