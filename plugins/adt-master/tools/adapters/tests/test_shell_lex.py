...
class ShellLexTests(unittest.TestCase):
    ...
    def test_quotes_escapes_and_operators(self):
        ...
        self.assertEqual(self.texts("K='a b' x"), [("K=a b", 2, False), ("x", False, False)])
        self.assertEqual(self.texts('K="a b" x'), [("K=a b", 2, False), ("x", False, False)])
        self.assertEqual(self.texts('"K=v" x'), [("K=v", True, False), ("x", False, False)])
        self.assertEqual(self.texts("a'b'c"), [("abc", 1, False)]); self.assertEqual(self.texts("'a'bc"), [("abc", True, False)]); self.assertEqual(self.texts("ab\\=c"), [("ab=c", 2, False)])
        self.assertEqual([shell_lex.unquoted_prefix(t, q) for t, q, _ in self.texts("K='a b' \"x\" y")], [2, 0, 1])
